    ...
    try {
      ...
      createdTime = DateFormat('HH:mm a').format(time);
       ...
       if (sendResponse.statusCode == 200) {
        ...
        // Check if the server response contains a 'success' or 'status' field
        if (paymentResponse.containsKey('success') && paymentResponse['success'] == true) {
          // Show the success dialog
          goToDialog(createdDate, createdTime);
        } else if (paymentResponse.containsKey('status') && paymentResponse['status'] == 'success') {
          // Show the success dialog
          goToDialog(createdDate, createdTime);
        } else {
          // If the server response does not indicate success, show the failure message
