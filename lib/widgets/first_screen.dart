...
class ScreenState extends State<Screen> with TickerProviderStateMixin {
  ...
  List<Widget> buildchildren(
    ...
    List<Widget> items = [];
    items.add(SizedBox(
      height: 10.0, // Adjusted height between title and list items
    ));
    items.add(Container(
      height: 40.0,
      child: Text(
    ...
    items.add(SizedBox(
      height: 20.0, // Adjusted height between title and list items
    ));
    for (int i = 0; i < a!.length; i++) {
      if (x == 0) {
        items.add(Container(
          padding: EdgeInsets.only(top: 10.0, bottom: 7.0), // Adjusted padding
          child: Expanded(
            child: Column(
        ...
        items.add(Container(
          padding: EdgeInsets.only(top: 10.0, bottom: 15.0), // Adjusted padding
          child: Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
      ...
      if (i < a.length - 1) {
        items.add(SizedBox(height: 16.0)); // Adjusted gap between items
      }
    }
    return items;
  }
  ...
  List<Widget> buildCards(List<Include>? whatlearns,
    ...
    return [
      Container(
        ...
        height: (whatlearns!.length * 95) * 1.2,
